        ...
        for entry in src {
            if let Ok(entry) = entry
                && entry
                    .file_name()
                    .to_str()
                    .is_some_and(|f| f.starts_with("year"))
            {
                let year = (&entry.file_name().to_str().unwrap()[4..])
                    .parse::<u32>()
                    ...
                    for year_entry in year_dir {
                        if let Ok(year_entry) = year_entry
                            && year_entry
                                .file_name()
                                .to_str()
                                .is_some_and(|f| f.starts_with("day"))
                            && year_entry
                                .file_name()
                                .to_str()
    ...
    for (year, days) in years {
        for (day, file_name) in days {
            function.push_str(&format!(
                "        ({year}, {day}) => year{year}_day{day}::run,\n"
            ));
            let path_str = abs_root_path
                .join("src")
            ...
            mods.push_str(&format!("pub mod year{year}_day{day};\n"));
        }
    }
